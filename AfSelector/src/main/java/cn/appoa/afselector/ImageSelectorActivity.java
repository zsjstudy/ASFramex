...
import cn.appoa.afselector.adapter.FolderAdapter;
import cn.appoa.afselector.adapter.ImageAdapter;
import cn.appoa.afselector.constant.Constants;
import cn.appoa.afselector.entry.Folder;
import cn.appoa.afselector.entry.Image;
import cn.appoa.afselector.model.ImageModel;
import cn.appoa.afselector.utils.DateUtils;
...
public class ImageSelectorActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.af_selector_activity_image_select);
    ...
    private void initView() {
        ...
        tvTitle.setText(isVideo ? isImage ? R.string.af_selector_image_and_video
                : R.string.af_selector_video : R.string.af_selector_image);
    ...
    private void setSelectImageCount(int count) {
        if (count == 0) {
            ...
            tvPreview.setText(R.string.af_selector_preview);
        } else {
            btnConfirm.setEnabled(true);
            btnPreview.setEnabled(true);
            tvPreview.setText(getResources().getString(R.string.af_selector_preview) + "(" + count + ")");
            if (isSingle) {
                tvConfirm.setText(R.string.af_selector_confirm);
            } else if (mMaxCount > 0) {
                ...
                tvConfirm.setText(getResources().getString(R.string.af_selector_confirm) + "(" + count + ")");
            }
        }
    }
    ...
    private void showExceptionDialog() {
        new AlertDialog.Builder(this).setCancelable(false).setTitle(R.string.af_selector_choose_hint)
                .setMessage(R.string.af_selector_no_read_camera_permission)
                .setNegativeButton(R.string.af_selector_cancel, new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        dialog.cancel();
                        finish();
                    }
                }).setPositiveButton(R.string.af_selector_confirm, new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                dialog.cancel();
                startAppSettings();
                            ...
                            if (isCamera) {
                                ...
                                if (folder.getImages() != null) {
                                    ...
                                    Image image = new Image(-1,
                                            ...
                                            getString(R.string.af_selector_camera));
                                    images.add(0, image);
                                }
                            }
                            initFolderList();
    ...
    private void takePhoto() {
        if (!Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
            ...
            mToast.setText(R.string.af_selector_not_found_camera_sd_card);
            mToast.show();
            return;
        }
        if (Build.VERSION.SDK_INT >= 24) {
